...
void calNBN_division(ofec::Problem* pro,
	...
	calNBN_iterater(pro, bestSolIds, bestId, sols, belong, dis2parent);
...
void calNBN(ofec::Problem* pro, ofec::Random* rnd,
	...
	calNBN_division(pro, bestSolIds, bestId, data.m_sols,
...
namespace ofec {
	...
	void run() {
		...
		{
			std::ofstream out(outputDir + std::string("CWIDN_nbn_") + UTILITY::getCurrentSystemTime() + ".txt");
			data.outputNBN(out);
			out.close();
		}

		{
			std::ofstream out(outputDir + std::string("CWIDN_nbn_sols_") + UTILITY::getCurrentSystemTime() + ".txt");
...

		{
			...
			std::ofstream out(outputDir + std::string("CWIDN_nbn_networks_") + UTILITY::getCurrentSystemTime() + ".txt");
