...
void solDataToNBN(
	...
	};
	{
		...
		edge_division.updateBestSolSub(2e3);
		int numThreadLoop = 12;
		int curLoop = 0;
		while (true) {
			...
			algInfo.output(algInfoOut);
			algInfo.output(std::cout);
			if (algInfo.m_maxChange < 0.1 && curLoop >= numThreadLoop) break;
...
namespace ofec {
	...
	void run() {
		...
		std::string outputDir = "/mnt/Data/Student/2018/YiyaDiao/NBN_data/nbn_visualization_tsp4/";
		std::string filename;
		//generateTSPdata();
		std::filesystem::create_directories(outputDir);
		{
			std::ofstream out(outputDir + "beginTxt.txt");
			out << "yes" << std::endl;
			out.close();
		}
