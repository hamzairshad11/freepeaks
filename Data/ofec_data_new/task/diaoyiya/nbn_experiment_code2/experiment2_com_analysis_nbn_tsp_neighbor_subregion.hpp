...
void solDataToNBN_subRegion(
	...
	const std::vector<int>& solId,
	const std::vector<std::shared_ptr<ofec::SolBase>>& sols,
	std::shared_ptr<ofec::SolBase>& centerSol,
	const std::string& tspname,
	const std::string& filename,
	ofec::Problem* pro,
	ofec::Random* rnd
...
namespace ofec {
	...
	void run() {
		...
		std::string outputDir = "/mnt/Data/Student/2018/YiyaDiao/NBN_data/nbn_visualization_tsp10/";
		outputDir = "/mnt/Data/Student/2018/YiyaDiao/NBN_data/nbn_visualization_tsp11/";
		outputDir = "/home/lab408/share/2018/diaoyiya/paper_com_experiment_data/tsp/tsp7/";
		//outputDir = "//172.24.24.151/e/DiaoYiya/paper_com_experiment_data/tsp_data/test5/";
		std::string filename;
		//generateTSPdata();
		{
		...
		std::vector<std::string> tsp_names2 = { "GR666","PCB442","XQL662","att532" };
		...
		auto solAroundId = solId;
		//	solAroundId.push_back(solId.size());
		std::vector<std::shared_ptr<ofec::SolBase>> samples;
		...
		for (auto& tspname : tsp_names) {
			...
			if (pro->optimaBase()->isVariableGiven()) {
				best_sol.reset(pro->createSolution(pro->optimaBase()->variableBase(0)));
			}
			else {
				for (auto& it : samples) {
			...
			/*if (tspname.front() == 'C' || tspname.front() == 'E') */
			{
			...
			int neighbor_k = pro->numVariables() / 4;
...

			{
				...
				while (neighbor_k > 5) {
					...
					//	int centerSolId = samples.size();
					//	samples.push_back(best_sol);
					for (auto& it : samples) {
						if (it->objective()[0] < best_sol->objective()[0]) {
							best_sol = it;
						}
					...
					solDataToNBN_subRegion(outputDir, solAroundId, samples,
						best_sol,
						tspname,
						filename, pro.get(), rnd.get());
					neighbor_k /= 4.0;
