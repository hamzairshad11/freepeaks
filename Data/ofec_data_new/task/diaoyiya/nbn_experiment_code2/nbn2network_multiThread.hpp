...
void readDir(const std::string& dir, std::vector<std::string>& names) {
	...
	for (auto const& dir_entry : std::filesystem::directory_iterator{ sandbox })
	{
		...
		if (curname.find("_nbn.txt") != std::string::npos) {
			int idx = curname.find("_nbn.txt");
			auto subname = curname.substr(0, idx);
...
void calTask(std::unique_ptr<utility::GeneralMultiThreadInfo>& curInfo,
...

	{
		...
		inputNBNdata(curGlobalInfo.m_read_dir + curcurInfo.m_filename + "_nbn.txt",
			nbn_data);
		...
		std::ofstream out(curGlobalInfo.m_save_dir + curcurInfo.m_filename + "_network.txt");
		nbn_graph.outputNBNnetwork(out);
		out.close();


		ouputNBNdata(curGlobalInfo.m_save_dir + curcurInfo.m_filename + "_nbn.txt", nbn_graph.get_nbn_data());
	}
...
void setThreadTask(const std::string& dir, utility::GeneralMultiThread& curThread) {
	...
	for (const auto& ssname : names) {
		...
		curInfo.m_taskname = filename;
		curInfo.m_error_name = "nbn_" + filename;
