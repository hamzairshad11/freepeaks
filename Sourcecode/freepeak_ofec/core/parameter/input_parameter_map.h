...
* 
* （1）修改parameter_map to variant_map
*  (2) 修改 	template<typename TValue = 规定几个类型>  InputParameterValueType 
* 
*
*********************************************************************************/
...
namespace ofec {
	...
	protected:
		...
		ParameterType m_paramType = ParameterType::Count;
	...
	public:
		...
		virtual void getValue(std::string& value) const {
			throw Exception("InputParameter getValue not defined");
		}
		virtual void setValue(const std::string& value) {
			throw Exception("InputParameter setValue not defined");
		}
	...
	public:
		InputParameterValueType(TValue &reference) : m_value(&reference) {
			m_paramType = getParameterType<TValue>();
		}
